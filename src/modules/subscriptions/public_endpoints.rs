    ...
    + crate::modules::subscriptions::validation::ValidationModule
{
    ...
    fn trigger_subscription(&self, id: u64) {
        ...
        let mut events_vec: MultiValueEncoded<SubscriptionMultiChargeResult<Self::Api>> =
        ...
        for account in members.iter() {
            ...
            if successful.is_some() || failed.is_some() {
                events_vec.push(SubscriptionMultiChargeResult::new(
                    &account, successful, failed,
                ));
            }
        }
    ...
    fn trigger_subscription_for_account(
        ...
        if affordable_cycles == 0 {
            return (
                None,
                self.charge_event_user_amount(total_pending_cycles, &cycle_cost),
            );
        }
