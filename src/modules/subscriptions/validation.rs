...
pub trait ValidationModule: crate::modules::subscriptions::storage::StorageModule {
    fn require_existing_subscription(&self, id: u64) {
        require!(
            !self.subscription_by_id(id).is_empty(),
            "Invalid subscription id."
        );
    }

    fn is_subscription_created_by_account(&self, id: u64, account: &ManagedAddress) -> bool {
        self.account_subscriptions_created_list(account)
            .contains(&id)
    }

    fn require_subscription_created_by_account(&self, id: u64, account: &ManagedAddress) {
        require!(
            self.is_subscription_created_by_account(id, account),
            "This subscription is not created by you."
        );
    }
    ...
    fn require_subscription_not_created_by_account(&self, id: u64, account: &ManagedAddress) {
        require!(
            !self.is_subscription_created_by_account(id, account),
            "This subscription is created by you."
        );
    }
    ...
    fn require_subscription_membership(&self, id: u64, account: &ManagedAddress) {
        require!(
            self.is_account_a_subscription_member(id, account),
            "Membership not active for this address."
        );
    }
    ...
    fn require_subscription_membership_not_exists(&self, id: u64, account: &ManagedAddress) {
        require!(
            !self.is_account_a_subscription_member(id, account),
            "This address has an active subscription membership."
        );
    }
    ...
    fn is_member_required_to_define_subscription_amount(
        &self,
